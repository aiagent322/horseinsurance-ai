...
export type ProcessJobDeps = {
  extract?: (bytes: Buffer) => Promise<ExtractedPdf>;
  now?: () => number;
};
...
export async function processClaimedJob(
  ...
  const extract = options.deps?.extract ?? ((bytes: Buffer) => extractPdfPages(bytes));
  ...
  try {
    ...
    for (let index = 0; index < job.files.length; index += 1) {
      ...
      try {
        extracted = await extract(original.bytes);
      } catch (err) {
      ...
      documents.push({
        ...
        analysis_status: extracted.extraction_status === "failed" ? "failed" : "complete",
    ...
    await assertLease(store, job, workerId, stage);
    const report = analyzeDocuments(job.policyId, job.sessionId, documents);

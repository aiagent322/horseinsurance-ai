...
function main(): void {
  ...
  assert.match(supabaseStore, /from\("analysis_jobs"\)/);
  assert.match(supabaseStore, /\.eq\("policy_id",\s*policyId\)/);
  assert.match(supabaseStore, /owner_user_id !== actor\.userId \|\| data\.account_id !== actor\.accountId/);
  assert.doesNotMatch(supabaseStore, /rpc\("get_own_job_status"/);
